...
Stdout carries only newline-delimited ACP JSON-RPC. `@phoenix-ai/dsh-acp-demo` installs no stdout logger; leaf additions must use stderr for diagnostics.

The automation contract — supported methods, baseline prompt content, committed-text output, and the intentionally absent UI surfaces — lives in [`@phoenix-ai/dsh-acp`](../../packages/acp/acp/README.md).
