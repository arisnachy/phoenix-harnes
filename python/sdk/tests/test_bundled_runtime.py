...
- id: session-checkpoints
  name: '@phoenix-ai/dsh-session-checkpoint-policy'
- id: subprocess
  name: '@phoenix-ai/dsh-subprocess-local'
- id: bash
  name: '@phoenix-ai/dsh-bash-local'
  config:
    cwd: '.'
- id: todo
...
def test_bundled_runtime_surfaces_unbundled_plugin_failure(tmp_path: Path, mode: str) -> None:
    ...
    (tmp_path / "cordis.yml").write_text(
        "- id: missing\n  name: '@phoenix-ai/dsh-does-not-exist'\n"
    )
    ...
    assert "@phoenix-ai/dsh-does-not-exist" in str(excinfo.value)
