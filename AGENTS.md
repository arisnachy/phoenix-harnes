...
PHOENIX is a plugin-based agent harness on vendored Cordis: **everything is a plugin**. Read [docs/architecture.md](docs/architecture.md) before changing `packages/`; follow [docs/AGENTS.md](docs/AGENTS.md) for documentation.
...
packages/    @phoenix-ai/dsh-<pkg> workspaces at packages/<group>/<pkg>/
...
native/      @phoenix-ai/node-addon-landlock-run source of record (see native/README.md)
...
- Every npm package is `@phoenix-ai/dsh-<name>`; vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@phoenix-ai/cordis` is a peerDependency (+ dev) of every harness package.
...
- **Green tests are evidence, never proof of completion.** For substantive generated or modified work, verify the real user-facing or production entrypoint and deliberately exercise the highest-value failure classes that apply: empty/one/many cardinality, boundary/extreme values, malformed input, deep/large input, dependency failure, timeout/cancellation, and lifecycle/resource cleanup. Prefer property/fuzz/metamorphic checks for broad input spaces. A directly observed failure overrides a passing suite. User-controlled input must produce a valid result or a domain-classified error; do not let raw implementation exceptions, tracebacks, partial writes, or silent corruption cross the user-facing boundary. Every discovered failure gets a root-cause repair plus a regression test that would have caught it, followed by a fresh run of the affected real entrypoint. Prefer cheap deterministic checks before model-based review, run independent checks in parallel when practical, and reuse still-fresh evidence while inputs are unchanged. Never silently substitute a lower-quality capability than the user requested. Spend additional verification only when expected quality gain justifies the added latency or cost. Use an independent verifier or fresh agent for substantial deliverables when practical; do not claim completion from self-authored unit tests alone.
