...
import { advanceAfter, advanceSkipped, dueOccurrences, initialNextRunAt, occurrenceId, userView } from './domain.ts'
import type {
...
export const Config: z<Config> = z.object({
  ...
  maxAttempts: z.number().step(1).min(1).default(5),
  retryBaseMs: z.number().step(1).min(1_000).default(60_000),
  retryMaxMs: z.number().step(1).min(1_000).default(3_600_000),
})
...
/** Durable owner-local task service with restart recovery and proactive execution. */
export class TaskService extends Service {
  static inject = ['agents', 'tools', 'subagents']
  ...
  constructor(ctx: Context, config: Config) {
    ...
    this.retryMaxMs = config.retryMaxMs ?? 3_600_000
    if (this.retryMaxMs < this.retryBaseMs) throw new RangeError('retryMaxMs must be greater than or equal to retryBaseMs')
  }
  ...
  /** Wait until persisted task state is loaded and queued mutations have settled. */
  async ready(): Promise<void> { await this.readyPromise; await this.mutationTail }

  /** Return user-visible task projections; private preparation tasks stay omitted by default. */
  ...
  /** Return durable occurrence history, optionally restricted to one task. */
  history(taskId?: string): TaskRunRecord[] {
    return this.store.runs.filter(run => taskId === undefined || run.taskId === taskId).map(run => ({ ...run }))
  }

  /** Create and persist one durable task. */
  create(input: CreateTaskInput): Promise<TaskRecord> {
    return this.serial(async () => {
      const task = this.createUnsafe(input)
      await this.persist()
      this.requestDrive(0)
  ...
  /** Create hidden advance-preparation tasks plus the final redacted reveal task. */
  createSurprise(input: Omit<CreateTaskInput, 'visibility' | 'origin' | 'schedule'> & { revealAt: string; prepareBeforeDays?: number[] }): Promise<TaskRecord[]> {
    return this.serial(async () => {
      const revealAt = parseIso(input.revealAt, 'revealAt')
      ...
      let preparationRootId: string | undefined
      for (const day of days) {
        const at = revealMs - day * 86_400_000
        if (at <= Date.now()) continue
        const record = this.createUnsafe({
          title: `Preparar sorpresa: ${input.title}`,
          prompt: `Prepara en privado la sorpresa "${input.title}". ${input.prompt}`,
          ...
          ...(preparationRootId === undefined ? {} : { parentTaskId: preparationRootId }), tags: ['surprise', 'preparation'],
        })
        preparationRootId ??= record.id
        records.push(record)
      }
      const reveal = this.createUnsafe({
        ...input, schedule: { kind: 'once', at: revealAt }, origin: 'phoenix', visibility: 'hidden_until_reveal',
        revealAt, parentTaskId: preparationRootId, tags: [...(input.tags ?? []), 'surprise', 'reveal'],
      })
      records.push(reveal)
      await this.persist()
      this.requestDrive(0)
  ...
  /** Pause or resume a task without deleting its durable history. */
  setState(id: string, state: Extract<TaskState, 'paused' | 'scheduled'>): Promise<boolean> {
    return this.serial(async () => {
      ...
      if (task === undefined || task.state === 'completed' || task.state === 'cancelled' || task.state === 'running') return false
      task.state = state
      if (state === 'scheduled') task.consecutiveFailures = 0
      task.updatedAt = new Date().toISOString()
      await this.persist(); this.requestDrive(0); return true
    })
  }

  /** Cancel a task and its private preparation children while preserving audit history. */
  cancel(id: string): Promise<boolean> {
    return this.serial(async () => {
      const targets = this.store.tasks.filter(task => task.id === id || task.parentTaskId === id)
      if (targets.length === 0) return false
      const now = new Date().toISOString()
      for (const task of targets) {
        if (task.state === 'completed') continue
        task.state = 'cancelled'; task.updatedAt = now
      }
      await this.persist(); this.requestDrive(0); return true
    })
  }

  /** Force an eligible task to become due immediately while keeping recurrence anchored. */
  runNow(id: string): Promise<boolean> {
    return this.serial(async () => {
      const task = this.store.tasks.find(candidate => candidate.id === id)
      if (task === undefined || task.state === 'running' || task.state === 'completed' || task.state === 'cancelled') return false
      task.state = 'scheduled'; task.nextRunAt = new Date().toISOString(); task.consecutiveFailures = 0; task.updatedAt = task.nextRunAt
      await this.persist(); this.requestDrive(0); return true
    })
  }

  private createUnsafe(input: CreateTaskInput): TaskRecord {
    ...
    const record: TaskRecord = {
      ...
      schedule, nextRunAt: initialNextRunAt(schedule, now), createdAt, updatedAt: createdAt, consecutiveFailures: 0,
      ...(input.revealAt === undefined ? {} : { revealAt: parseIso(input.revealAt, 'revealAt') }),
  ...
  private registerTools(agent: Agent): () => void {
    ...
    disposers.push(agent.ctx.tools.register(defineTool({
      ...
      description: 'Prepare a private Phoenix surprise before reveal_at in isolated background subagents, then reveal it at the requested time.',
    ...
    disposers.push(agent.ctx.tools.register(defineTool({
      name: 'task_list', description: 'Open the durable task center data. Internal preparation tasks are omitted and unrevealed surprises are redacted.', parameters: {},
      output: { schema: arraySchema, render: jsonOutput }, execute: async () => this.list(), presentCall: () => ({ card: 'generic', title: 'Task center', kind: 'read' }),
    })))
    disposers.push(agent.ctx.tools.register(defineTool({
    ...
    for (const [name, description, action] of [
      ['task_pause', 'Pause a durable task.', (id: string) => this.setState(id, 'paused')],
      ['task_resume', 'Resume a paused or failed durable task.', (id: string) => this.setState(id, 'scheduled')],
      ['task_cancel', 'Cancel a durable task without deleting its history.', (id: string) => this.cancel(id)],
      ['task_run_now', 'Run an eligible durable task as soon as possible.', (id: string) => this.runNow(id)],
    ] as const) {
      disposers.push(agent.ctx.tools.register(defineTool({
        name, description, parameters: { id: { type: 'string', required: true } }, output: { schema, render: jsonOutput },
        execute: async args => ({ id: args.id, ok: await action(args.id) }), presentCall: () => present(description),
      })))
    }
  ...
  private async load(): Promise<void> {
    try {
      ...
      for (const run of this.store.runs) {
        ...
        run.status = 'failed'; run.finishedAt = now; run.retryAt = now
        run.error = 'Phoenix restarted before the occurrence settled; retry is eligible after recovery.'
        interruptedTaskIds.add(run.taskId)
      }
      for (const task of this.store.tasks) {
        if (task.state === 'running' || interruptedTaskIds.has(task.id)) {
          task.consecutiveFailures += 1
          task.state = task.consecutiveFailures >= this.maxAttempts ? 'failed' : 'scheduled'
          task.updatedAt = now
        }
      }
    } catch (error: unknown) {
  ...
  private settleSkipped(now: number): boolean {
    let changed = false
    for (const task of this.store.tasks) {
      if (task.state !== 'scheduled' || task.catchUp !== 'skip' || Date.parse(task.nextRunAt) >= now) continue
      const dueAt = task.nextRunAt
      const id = occurrenceId(task.id, dueAt)
      const exists = this.store.runs.some(run => run.occurrenceId === id && (run.status === 'completed' || run.status === 'skipped'))
      if (!exists) {
        const at = new Date(now).toISOString()
        this.store.runs.push({ occurrenceId: id, taskId: task.id, dueAt, status: 'skipped', startedAt: at, finishedAt: at })
      }
      Object.assign(task, advanceSkipped(task, now))
      changed = true
    }
    return changed
  }

  private surpriseRevealFor(task: TaskRecord): TaskRecord | undefined {
    if (!task.tags?.includes('preparation')) return undefined
    const rootId = task.parentTaskId ?? task.id
    return this.store.tasks.find(candidate => candidate.tags?.includes('reveal') && candidate.parentTaskId === rootId)
  }

  private async executeInternal(task: TaskRecord, agent: Agent, signal: AbortSignal): Promise<void> {
    const provider = this.ctx.subagents.list().find(name => name === 'fork') ?? this.ctx.subagents.list().find(name => name === 'spawn') ?? this.ctx.subagents.list()[0]
    if (provider === undefined) throw new Error('No subagent provider is available for private task execution')
    const run = await this.ctx.subagents.start(provider, {
      label: task.title,
      parent: agent,
      signal,
      prompt: [{ type: 'text', text: `[PHOENIX PRIVATE TASK]\n${task.prompt}\nWork privately. Do not report or message the user. Produce preparation material for the later reveal task and use only currently authorized tools and permissions.` }],
    })
    try {
      const result = await run.result
      if (result.stopReason !== 'completed') throw new Error(result.diagnostic ?? `private task ended with ${result.stopReason}`)
      const reveal = this.surpriseRevealFor(task)
      if (reveal !== undefined && result.output.length > 0) {
        reveal.prompt += `\n\n[Private preparation from ${task.id}]\n${JSON.stringify(result.output)}`
        reveal.updatedAt = new Date().toISOString()
      }
    } finally {
      await run.dispose()
    }
  ...
  private async executeVisible(task: TaskRecord, targetDueAt: string, agent: Agent): Promise<void> {
    let intervalError: unknown
    const stopError = agent.ctx.on('agent/error', ({ agent: failedAgent, error }) => {
      if (failedAgent === agent && intervalError === undefined) intervalError = error
    })
    try {
  ...
  private async drive(): Promise<void> {
    ...
    await this.serial(async () => {
      const now = Date.now()
      if (this.settleSkipped(now)) await this.persist()
      ...
      try {
        if (task.visibility === 'internal') await agent.runMaintenance(signal => this.executeInternal(task, agent, signal))
        else await this.executeVisible(task, target.dueAt, agent)
        run.status = 'completed'; run.finishedAt = new Date().toISOString(); delete run.retryAt
        Object.assign(task, advanceAfter({ ...task, state: 'scheduled' }, target.dueAt, Date.now()), { consecutiveFailures: 0 })
      } catch (error: unknown) {
        task.consecutiveFailures += 1
        const failedAt = Date.now()
        run.status = 'failed'; run.finishedAt = new Date(failedAt).toISOString(); run.error = error instanceof Error ? error.message : String(error)
        if (task.consecutiveFailures >= this.maxAttempts) {
          task.state = 'failed'
        } else {
          task.state = 'scheduled'
          run.retryAt = new Date(failedAt + this.retryDelay(task.consecutiveFailures)).toISOString()
        }
        task.updatedAt = new Date(failedAt).toISOString()
      }
      await this.persist()
    })
    const now = Date.now()
