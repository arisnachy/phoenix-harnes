...
`@phoenix-ai/dsh-authorization/google` is a protocol-owning Host Service layered beside the neutral seam. It registers one `Google Workspace` flow and uses Google's Desktop-app authorization-code flow with a random loopback listener, PKCE S256, and `state`. The Desktop OAuth `client_id` identifies the application; it is not a user password or OAuth token. On the Host, PHOENIX first looks for the existing KIRA/OpenClaw Desktop client at `~/.dsh/secrets/google-oauth-client.json`, reads only its `client_id`, then falls back to an explicitly composed `clientId` and finally `PHOENIX_GOOGLE_OAUTH_CLIENT_ID`. A present but malformed local client file fails loud with a sanitized diagnostic; its `client_secret` is never copied into broker config, telemetry, prompts, or model context.

Google access and refresh tokens are deliberately **process-local**. After successful authorization the broker keeps them only in private Host memory and commits a secret-free `{ kind: 'api-key' }` marker under `authorization-google/account` so the neutral authorization seam can observe that the human flow committed. Browser-facing `list()`, `describe()`, `inspect()`, credential metadata, config, environment, and the marker itself contain no OAuth token, authorization code, or PKCE verifier. A PHOENIX restart therefore still requires Google authorization again, but the first Google capability request can start that ceremony automatically: PHOENIX opens the system browser to the generated Google authorization URL, receives the result on the loopback callback, and then continues the original API request. If an on-demand ceremony fails or the user explicitly disconnects, automatic reopening is suppressed until an explicit successful reconnection so persistent agents cannot create a popup loop.
...
None, as authorization is a Host-owned conversation with the human and no flow, notice, prompt, OAuth token, credential payload, broker authentication header, or local OAuth client file enters a model request; model-visible Google tools receive only the Google API data their owning integration chooses to return.
...
- **Google login is process-local by design** — a restart discards access and refresh tokens. The next Google capability can automatically start a fresh browser ceremony, but durable login remains disabled until an OS-isolated credential broker exists. Persisting the refresh token in `credentials-local` would expose it to deliberately malicious same-UID processes.
