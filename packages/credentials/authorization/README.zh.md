...
`@deepseek-ai/dsh-authorization/google` 是位于中立 seam 旁边、由协议所有者实现的 Host Service。它注册一个 `Google Workspace` flow，并采用 Google Desktop 应用的授权码流程：随机 loopback listener、PKCE S256 与 `state`。配置中的 `clientId` 只标识 OAuth 应用，并不是用户机密；发布组合从 `PHOENIX_GOOGLE_OAUTH_CLIENT_ID` 读取它。

broker 刻意**不**通过 `ctx.credentials` 持久化 Google access token 或 refresh token。当前本地凭据 provider 已明确说明：其仅所有者可读的文件权限并不能隔离以同一 OS 用户运行的模型工具进程，因此把 Google grant 存进去会削弱本 provider 要保障的安全属性。登录成功后，store 只收到不含机密的 `authorization-google/account` marker。access/refresh token 仅存在于 Host Service 的私有字段，进程退出即消失；所以在 PHOENIX 具备同 UID 工具进程无法访问的凭据后端之前，重启后必须重新进行 Google 授权。

调用方使用 `ctx.googleApi.request({ url, requiredScopes, ... })`。broker 只接受 HTTPS Google API 目标，拒绝 OAuth token endpoint 以及调用方自己提供的 authorization/cookie header；每次操作都按实际 grant 验证其 required scopes，仅在 `fetch` 前一刻注入 Bearer token，并只返回状态、媒体类型和 body，而不会转发可能携带凭据的响应状态。`disconnect()` 即使 Google 的 best-effort revoke 请求失败，也会清掉内存 session 与 durable marker。

Google installed application 不支持 incremental authorization，因此挂载的 provider 会在一次人工 consent 中请求其配置的 scope 集合。部署应只保留实际启用能力所需的 scope，并为 sensitive/restricted scope 完成 Google 的 OAuth 验证要求。

## Model Experience

无，因为授权是配置期与人的对话，flow、notice、prompt、OAuth token 与 broker 凭据都不会抵达模型请求。
...
- **Google session 按设计只存在于进程内** —— Google broker 只持久化 marker，因为当前本地凭据文件并不是同 UID 进程之间的安全边界。在更强的 secret backend 可用之前，重启后需要重新授权。
