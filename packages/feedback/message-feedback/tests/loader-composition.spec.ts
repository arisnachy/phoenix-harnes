...
import * as StorageDomain from '@phoenix-ai/dsh-storage-domain'
import * as StorageJson from '@phoenix-ai/dsh-storage-json'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@phoenix-ai/dsh-session', SessionStore],
    ['@phoenix-ai/dsh-session-persistence-jsonl', JsonlSessionPersistence],
    ['@phoenix-ai/dsh-storage', Storage],
    ['@phoenix-ai/dsh-storage-json', StorageJson],
    ['@phoenix-ai/dsh-storage-domain', StorageDomain],
    ['@phoenix-ai/dsh-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    ...
    await writeFile(configPath, [
      "- name: '@phoenix-ai/dsh-session'",
      "- name: '@phoenix-ai/dsh-session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@phoenix-ai/dsh-storage'",
      "- name: '@phoenix-ai/dsh-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@phoenix-ai/dsh-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@phoenix-ai/dsh-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
