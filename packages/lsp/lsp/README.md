...
| Package | Role |
|---|---|
| `@phoenix-ai/dsh-lsp` (this) | Service Definition: the service, provider registry keyed by branded id + extension mapping, per-query selection, request/result vocabulary, the `LspError` taxonomy |
| `@phoenix-ai/dsh-lsp-stdio` | Service Provider: a generic local backend that registers configured stdio language-server providers |
| `@phoenix-ai/dsh-tool-lsp` | Consumer: the model-facing `lsp` tool over `ctx.lsp` |
