...
declare module '@phoenix-ai/dsh-jobs' {
...
/**
 * Unit coverage for the @phoenix-ai/dsh-agent-spine-demo bundle: mounting it brings
...
describe('dsh-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@phoenix-ai/dsh-agent$'] },
      { package_blocklist: ['^@phoenix-ai/dsh-session$'] },
    ]) {
  ...
  it('rejects conflicting global and nested PHOENIX home directories', () => {
    expect(() => {
