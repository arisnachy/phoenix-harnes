# @phoenix-ai/dsh-agent-spine-demo
...
@phoenix-ai/dsh-system-prompt    prompt-section + tool-schema assembly
@phoenix-ai/dsh-tools            registry + guarded pre/around/post/final-result pipeline
@phoenix-ai/dsh-skill            skill provider registry
@phoenix-ai/dsh-skill-filesystem      local filesystem skill provider
@phoenix-ai/dsh-agent            agent registry + initiator scope + agent/* events
@phoenix-ai/dsh-goal             optional persisted same-session goal domain
@phoenix-ai/dsh-tool-goal        optional model-facing goal controls
@phoenix-ai/dsh-goal-round-driver     optional same-session goal-round driver
@phoenix-ai/dsh-llm-retry        provider-routed request retry policy
@phoenix-ai/dsh-jobs-local      generic background-job registry
@phoenix-ai/dsh-invariants       configurable invariant registry service
@phoenix-ai/dsh-session/invariant
@phoenix-ai/dsh-agent/invariant
@phoenix-ai/dsh-scope/invariant
@phoenix-ai/dsh-agent-loop/invariant
                                  package-owned relational checks
@phoenix-ai/dsh-tool-bash        the model-facing bash schema (unless toolBash=false)
@phoenix-ai/dsh-agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@phoenix-ai/dsh-tool-skill       session-prefix skill catalog + model-facing loader schema
@phoenix-ai/dsh-tool-jobs       job_output/job_list/job_kill schemas + completion notices
@phoenix-ai/dsh-agent-loop       THE concrete loop (gets the forwarded `agents`)
...
```ts
import type { Config } from '@phoenix-ai/dsh-agent-spine-demo'
...
例如，`{ invariants: { enabled: true, package_allowlist: ['^@phoenix-ai/dsh-'], package_blocklist: ['agent-loop$'] } }` 会让包拥有的配套插件保持挂载，但抑制被阻止的拥有者。Blocklist 匹配优先于 allowlist 匹配；正则表达式与生命周期规则见 [`dsh-invariants`](../../runtime-diagnostics/invariants/README.zh.md)。
