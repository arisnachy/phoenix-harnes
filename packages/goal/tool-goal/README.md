...
When `requireJudge` is enabled, `complete` first runs the adversarial verifier and then starts a fresh structured subagent with a read-only tool allow-list. Each verifier stage and the final judge have a bounded ten-minute attempt timeout. The goal enters `complete` only after the judge returns `pass`; `needs_changes` keeps it active, appends the required changes to the session, and allows the next bounded goal round to repair the work. Infrastructure failures remain attempt-level: the result reports whether a stage failed before launch, after launch, by stop reason, by timeout, by invalid structured output, or during cleanup, and the active mission retries without demanding a special resume phrase. Judge results are persisted as secret-free `goal/judge` events while detailed verifier incidents remain in the completion-gate evidence and deferred continuation context.
