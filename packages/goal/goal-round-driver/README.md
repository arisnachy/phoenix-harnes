...
The retained prompt names the JSON-quoted objective and `round/maxGoalRounds`, treats the current workspace, tool results, and durable session state as authoritative, requires evidence before completion, and tells the model to use a materially different strategy after the first unsuccessful round. A question result marked `automatic: true` is explicitly a step decision only; it cannot complete, cancel, or block the active mission, and the model must continue or change strategy. Explicit pending, remaining, TODO, or not-yet-completed requirements are mission debt: the next round must resolve them instead of presenting them as a final handoff. When a technical tool or capability is absent, the prompt requires ATLAS/connector inspection, a materially different route, then governed acquisition or construction plus testing when the runtime permits it. It leaves the goal active when work remains. Quoting preserves multiline or tag-like objective text as data. Goal lifecycle mutations still require the independent authority checks in `dsh-tool-goal`.

## Mission-debt stop fence

An ordinary direct-human turn can enter the persistent goal lifecycle even when the model forgot to call `create_goal`. At the existing `agent/turn-stopping` extension point, the driver inspects only the current root-agent turn. It bootstraps a goal only when all three facts are durable: the current turn contains direct human input, at least one real `tool/call`, and the final assistant text explicitly states unresolved work such as `Pendiente:`, `Pending:`, or `not yet`. Text-only conversations and tool-assisted answers that state no remaining debt stop normally. The exact human request becomes the durable objective; the assistant debt line is detection evidence only and never replaces the objective.

The fence is deliberately narrow. It does not infer unfinished work from uncertainty, generic future suggestions, or ordinary conversational language, and plugin-authored messages cannot grant the direct-human authority needed to bootstrap a mission. Once created, the ordinary goal driver owns continuation, durability, strategy rotation, adversarial completion verification, and the independent completion judge.
