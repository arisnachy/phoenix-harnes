...
Cancellation records its durable cause on an admitted goal attempt. At the next idle checkpoint, only an explicit user stop with no replacement inbox work pauses the goal. Interactive steering also uses a user abort internally, but because it queues replacement input first the mission remains active, yields to that input, and resumes automatically afterward. Hook, parent, verifier/transport, stale-reservation, and other attempt-level interruptions clear the failed reservation, checkpoint `retrying/continue`, and schedule a fresh goal round instead of requiring a human resume or a magic phrase. Cancellation unrelated to a goal attempt removes process-local authority only when it is an explicit user stop. If the pause mutation fails, the driver falls back to disarming. Plugin teardown closes admission, disarms every live goal, cancels active work with the `parent` cause, and awaits the driver plus agent quiescence while its event fence remains installed.
