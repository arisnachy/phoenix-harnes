...
`ctx.goals.organizationForge` is an event-backed `OrganizationForgeLedger` for organization and product builds. It records research sources, pre-reuse and post-modification audits, required acceptance criteria, evidence, the independent judge result, and the selected handoff or management mode. A Forge build cannot reach `ready` until every required criterion is verified and every reused source has passed both audits.
