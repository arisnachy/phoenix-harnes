# @phoenix-ai/dsh-attachment-local

English | [中文](README.zh.md)

The private local implementation of [`@phoenix-ai/dsh-attachment`](../attachment). Images and arbitrary files land at `<DSH_HOME>/attachments/v1/objects/<sha256-prefix>/<sha256>` and are addressed by an opaque `sha256:` id. Each process proves a home durable once by syncing every ancestor entry to the filesystem root. Writes use a private staging directory, owner-only files, a synced temporary file, an atomic exclusive hard-link publish, and directory syncs on the publication path (POSIX; Windows relies on filesystem metadata journaling) so the reported reference survives a crash.

Arbitrary files are admitted up to 25 MiB each, 20 files per message, and 100 MiB per message by default. Their MIME type is normalized, their display name is reduced to one path-free leaf, and the stored digest and byte count are checked on every read. The same content-addressed object can be reused by resumed sessions and exports without copying it.
