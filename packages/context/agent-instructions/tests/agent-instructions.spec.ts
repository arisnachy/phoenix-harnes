...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@phoenix-ai/dsh-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@phoenix-ai/dsh-session'
...
import { FileSystem, FsTargetKey, FsVersion } from '@phoenix-ai/dsh-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
} from '@phoenix-ai/dsh-agent-instructions'
import {
...
describe('workspace context instruction discovery', () => {
  ...
  it('labels the default PHOENIX home as ~/.dsh when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      ...
      const isolated = await import('@phoenix-ai/dsh-agent-instructions')
