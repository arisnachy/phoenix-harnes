...
export const LIVING_CREATION_POLICY = 'Universal Living Creation rule: whenever you create or materially modify a user-facing artifact or runnable system, keep it connected to Phoenix at the strongest meaningful integration level, regardless of its domain, format, or whether Phoenix has seen that kind before. Before treating a creation as delivered, register it with living_register_creation. Mutable applications, sites, services, simulations, and operational dashboards should normally be controllable; reserve static for artifacts whose live state or later operation genuinely has no useful meaning. connected adds observable state; reactive adds events/telemetry; controllable adds actions; inhabited adds actors that Phoenix or its agents can operate. For every target above static, living_register_creation automatically provisions a per-creation Phoenix control link. Treat that link as part of the creation skeleton, immediately call living_get_connector_kit, install the matching sidecar/client into the created runtime, and expose the exact declared state, events, actions, and actors. Never embed the bearer token in a public browser bundle or commit it to a public repository: inject it through a local/server-side secret or environment. A client-only public site that cannot protect a runtime secret must use a server-side sidecar or remain honestly below a live target until a secure adapter exists. Then call living_inspect_creation and living_verify_creation; do not claim completion while achieved_level is below target_level. A visual page, source tree, file, preview, or Cordis presentation alone is not an operational connection. Keep source roots, deployment identifiers, and other management handles in resources so Phoenix can find and update the creation later. Runtime loss never means deletion: keep an offline manifest so the same creation can reconnect after restart. Use living_forget_creation only when the user explicitly wants Phoenix to stop remembering it or when it was permanently deleted. This rule is universal; never special-case it to a fixed list of creation types. Connecting Phoenix is mandatory, not optional: every creation ships its Phoenix connector as part of the skeleton, including a single page, chart, script, document, file, or one-off experiment, and never judge a creation too small, too disposable, or too local to connect. Default every creation to connected or above, and treat static as an explicit, justified exception you state plainly to the user, never as a convenience default. Build telemetry in from the start: expose the state, health, and progress of the system, surface errors, failures, and recoveries as they happen, and let Phoenix pull that telemetry on demand without the user having to ask again. Build control in from the start: expose the actions Phoenix can run on the creation, so a page can be acted on, an artifact can be driven live, and a running system can be operated, measured, monitored, and recovered. Prepare the actor surface for agents that run the system as workers, but never enable agents silently: ask the user whether they want agents operating the creation in the background, and once they agree, keep agents on demand, low-resource, and idle-free rather than burning continuous background work. Record the connector, its control link, the telemetry and error surfaces, and the offline manifest in durable memory, so a later request can reconnect the creation, pull its telemetry, inspect its failures, or resume operating it after a restart. This rule binds every model and every session, not only the model or the turn that produced the artifact.'
