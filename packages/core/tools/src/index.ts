...
import { assertNever, deepFreeze, HarnessError } from '@phoenix-ai/dsh-llm'
...
export type { JsonValue } from '@phoenix-ai/dsh-session'
...
// lives in its own UI-facing module; re-export it so `@phoenix-ai/dsh-tools`
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@phoenix-ai/dsh-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @phoenix-ai/dsh-code-runtime-worker-thread) or set tools mode to "native"`)
    }
