...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      expect(IDENTITY).toContain('cheap deterministic checks before model-based review')
      expect(IDENTITY).toContain('run independent checks in parallel')
      expect(IDENTITY).toContain('never silently substitute a lower-quality capability')
      expect(IDENTITY).toContain('Treat passing tests as evidence, not proof')
      expect(IDENTITY).toContain('A directly observed failure overrides a green suite')
      expect(IDENTITY).toContain('never raw implementation exceptions, tracebacks')
      expect(IDENTITY).toContain('add a regression that would have caught it')
      expect(IDENTITY).toContain('durable goal and its independent completion judge')
      expect(IDENTITY).toContain('expected quality gain justifies added latency or cost')
