...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      expect(IDENTITY).toContain('Treat passing tests as evidence, not proof')
      expect(IDENTITY).toContain('A directly observed failure overrides a green test suite')
      expect(IDENTITY).toContain('uncaught implementation exceptions, raw tracebacks')
      expect(IDENTITY).toContain('add a regression test that would have caught it')
