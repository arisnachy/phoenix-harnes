...
| `startupTimeoutMs` | both | no | Maximum initial connection and tool-discovery wait before optional boot continues in the background (default 5000) |
...
- **Startup timeout covers activation, not every SDK request** — `startupTimeoutMs` prevents an optional connector from blocking host activation. The MCP SDK may still keep an individual initialize or paginated `tools/list` request alive while the supervisor disposes or retries that connection.
