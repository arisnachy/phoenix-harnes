...
import * as TerminalLocal from '@phoenix-ai/dsh-terminal-bash'
...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    ...
    await writeFile(configPath, [
      ...
      '    timeoutMs: 2000',
      '    disposeGraceMs: 500',
      "- name: '@phoenix-ai/dsh-tool-bash-persistent'",
      '  config:',
      '    timeoutMs: 5000',
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@phoenix-ai/dsh-subprocess-local', LocalSubprocessRuntime],
      ['@phoenix-ai/dsh-terminal-bash', TerminalLocal],
      ['@phoenix-ai/dsh-tool-bash-persistent', ToolBashPersistent],
    ])
