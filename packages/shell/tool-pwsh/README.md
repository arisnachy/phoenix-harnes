...
The Windows composition also registers the guarded `computer` tool. In Phoenix Desktop its structured browser actions operate the embedded WebView2 directly: `browser_inspect` returns visible text, field metadata and buttons without returning current field values; `browser_fill_form` fills inspected non-secret fields; and `browser_click_text` activates a visible button/link by text. `browser_login` asks through a private Phoenix prompt the first time an HTTPS origin needs credentials, saves them in the local Phoenix Vault by default, and fills one unambiguous visible account/password pair on later requests without submitting the form. The user can turn off saving in the prompt, and `browser_forget_credentials` removes saved credentials for the current origin. Credentials never appear in Computer arguments, results, screenshots returned for the login action, or learned flow data. `browser_fill_form` rejects password and file controls.

When Phoenix Desktop publishes `PHOENIX_DESKTOP_CONTROL_DESCRIPTOR`, every Computer action uses the resident schema-2 Win32 driver over one current-user named pipe. The server accepts only the owned runtime process tree, serializes requests, correlates replies by `requestId`, and closes the channel on timeout or cancellation. State-changing requests include a screenshot captured immediately after input injection returns, avoiding a separate screenshot request and fixed post-action delay; credential actions suppress screenshots. The model must verify the visible outcome because tool success confirms OS input, not application completion. Outside Phoenix Desktop, the validated PowerShell driver remains the fallback. This removes process overhead but does not claim timing parity with Codex without a same-machine benchmark.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@phoenix-ai/dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls on Windows (ConPTY) and POSIX hosts with pwsh.
