...
固定 PowerShell/C# driver 通过 stdin 传输，以避开 Windows 命令行长度限制。模型控制的文本和 selector 只通过 `PHX_*` 环境变量传递，绝不成为可执行源码。`read-only` 允许 `screenshot` 与 `windows`；`workspace-write` 通过普通 approval service 路由输入；`danger-full-access` 是明确的无提示桌面权限。除鼠标移动外，每个改变状态的动作都会短暂等待、捕获新截图并将图像延迟注入模型上下文。成功结果表示 Windows 接受了该操作；模型必须用新观察验证应用层结果。
...
模型在相应工具可用时看到生成的 [`computer`](../../../docs/tool-catalog.zh.md#computer) 与 [`pwsh`](../../../docs/tool-catalog.zh.md#pwsh) schema。按 agent 作用域的工具限制可以移除任一定义。Computer 结果返回所选动作、操作系统层状态、可选窗口明细，以及是否附加了新截图；可见窗口枚举为每个窗口返回一条确定性文本。
...
- **无持久 shell** — 每次调用都启动全新的 `pwsh -Command`；持久 shell 对应物是 [`@phoenix-ai/dsh-tool-pwsh-persistent`](../tool-pwsh-persistent/README.zh.md)，它在 Windows（ConPTY）以及装有 pwsh 的 POSIX 主机上跨调用保持一个 owner 作用域的 pwsh 存活。
