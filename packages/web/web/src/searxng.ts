/** Harness-owned SearXNG provider; independent from the selected LLM provider. */

import type { WebSearchProvider, WebSearchRequest, WebSearchResult, WebSearchSource } from './types.ts'
...
interface SearxngResponse { results?: unknown }
...
/** Normalize SearXNG's JSON API response into the PHOENIX web seam. */
export function mapSearxngResponse(response: SearxngResponse): WebSearchResult {
  if (!Array.isArray(response.results)) throw new WebError('SearXNG returned an invalid results payload', 'WEB_PROVIDER_ERROR')
...
/** Keyless SearXNG provider. No model/API credential is ever resolved here. */
export class SearxngSearchProvider implements WebSearchProvider {
  ...
  available(): boolean {
    return URL.canParse(this.resolveBaseURL())
  }

  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    let endpoint: URL
    try {
      endpoint = new URL('/search', normalizedBase(this.resolveBaseURL()))
    ...
    if (!response.ok) throw new WebError(`SearXNG search failed (HTTP ${response.status})`, 'WEB_PROVIDER_ERROR')
    try {
