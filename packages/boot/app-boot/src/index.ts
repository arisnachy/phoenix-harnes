 ...
 * @module @phoenix-ai/dsh-app-boot
 */
...
import { Context, type FiberState } from '@phoenix-ai/cordis'
import Loader, { type Entry, type EntryOptions } from '@phoenix-ai/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@phoenix-ai/cordis-plugin-include'
import Group from '@phoenix-ai/cordis-plugin-group'
import { dshHomePath, resolveDshHome } from '@phoenix-ai/dsh-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@phoenix-ai/dsh-launch-environment'
import type {} from '@phoenix-ai/cordis-plugin-hmr'
 ...
 * entries (`@phoenix-ai/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@phoenix-ai/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@phoenix-ai/cordis-plugin-group`
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The PHOENIX implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend PHOENIX itself.`,
  })
}
