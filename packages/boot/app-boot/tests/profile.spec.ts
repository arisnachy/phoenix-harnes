...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@phoenix-ai/dsh-base'])
    ...
    expect(readProfileManifest('t', dir).dsh?.profile?.bundles).toEqual(['@phoenix-ai/dsh-base'])
...
describe('loadProfile', () => {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@phoenix-ai/dsh-base': { patch: '[]\n' },
      '@phoenix-ai/dsh-web-app': { patch: '[]\n' },
      '@phoenix-ai/dsh-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    ...
    expect(readProfileManifest('t', stock).dsh?.profile?.bundles)
      .toEqual(['@phoenix-ai/dsh-base', '@phoenix-ai/dsh-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@phoenix-ai/dsh-base', '@phoenix-ai/dsh-web-app', '@phoenix-ai/dsh-headless', 'custom-bundle',
    ])
