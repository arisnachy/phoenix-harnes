...
describe('plugin registration and config', () => {
  ...
  it('owns the deepseek provider and advertises the default models', async () => {
    ...
    await expect(ctx.llm.listModels('deepseek-official')).resolves.toEqual([
      ...
      { provider: 'deepseek-official', id: 'deepseek-v4-flash', name: 'DeepSeek-V4-Flash (legacy alias)', inputModalities: ['text', 'image'] },
      { provider: 'deepseek-official', id: 'deepseek-v4-pro', name: 'DeepSeek-V4-Pro', inputModalities: ['text'] },
      { provider: 'deepseek-official', id: 'deepseek-v4-flash-vision-exp', name: 'DeepSeek-V4-Flash-Vision-Exp (legacy alias)', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('deepseek-official', 'deepseek-v4-flash'))
      .resolves.toMatchObject({
        provider: 'deepseek-official',
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    ...
    await expect(ctx.llm.listModels('deepseek-official')).resolves.toEqual([
      { provider: 'deepseek-official', id: 'deepseek-v4.1-flash', name: 'DeepSeek-V4.1-Flash', inputModalities: ['text'] },
  ...
  it('keeps the old PHOENIX V4.1 selector as a hidden compatibility alias and dispatches canonical Flash', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const ctx = await harness(server.url)
  ...
  it('accepts image input on canonical DeepSeek Flash without the legacy vision-only selector', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const attachments = attachmentStoreOf(ref => Promise.resolve(requestImage(ref))).store
    const files = fileStoreOf(() => Promise.reject(new LlmError('Files unavailable', 'TRANSPORT')))
    const adapter = adapterOf({ baseURL: server.url }, attachments, files.store)
    ...
    expect(JSON.stringify(server.requests[0])).toContain('"model":"deepseek-flash"')
