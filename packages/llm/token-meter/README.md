...
`tokenUsage` carries the complete durable log's `uncachedInputTokens`, `outputTokens`, `cacheReadTokens`, and `cacheWriteTokens`, including separately reported compaction calls. Usage chunks are counted even when a request later fails; a final assistant-message usage for the same `(turn, step)` replaces that sample instead of double-counting it. Reasoning remains an output subdivision. This is observed request usage, not an account balance or provider quota. The single last-sample slot relies on a session-log ordering property: once a later step reports usage, a legal log never reports usage for an earlier step again.
...
```yaml
- name: '@phoenix-ai/dsh-token-meter'
- name: '@phoenix-ai/dsh-compaction-basic'
```
