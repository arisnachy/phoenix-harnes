...
The configurable-provider directory also includes the optional `chatgpt-web` route for a locally running [`codex-chatgpt-web`](https://github.com/miuuyy/codex-chatgpt-web) bridge. Adding `providers.chatgpt-web: {}` activates Phoenix's OpenAI Responses adapter with the bridge defaults (`http://127.0.0.1:17841/v1`, `gpt-5.6-sol`, and `gpt-5.6-luna`); an empty `models` list is treated the same way because the settings loader materializes omitted lists as empty. Overriding `baseURL` or providing a non-empty `models` list remains supported. The route does not import browser cookies, credentials, or a model API key; it sends a non-secret loopback authorization marker only because pi-ai's OpenAI Responses preflight requires an auth header, while the bridge authenticates the browser session. Set `PHOENIX_CHATGPT_WEB_URL` to the bridge origin and run `dsh doctor` to check its `/v1/models` endpoint without printing response data.

The `image_generation` tool persists each successful Codex raster in the durable attachment store and returns its absolute local path as well as the attachment reference. The model can pass that path to `read_image` on a later step when it needs to inspect or embed the generated asset in another deliverable; the path is local host metadata, not a bearer URL.
...
User file attachments are resolved from the durable attachment store on every request. Text-like files are included as bounded UTF-8 text with their name, media type, byte count, and truncation marker; binary files remain visible as named descriptors without attempting to decode arbitrary bytes. `maxInlineFileBytes` defaults to 256 KiB and limits only text projection, so an attached file never silently disappears from the model request.
...
The selected catalog model receives `GenerateOptions.system`, history, tools, and sampling fields supported by pi-ai's common streaming API. For `openai-codex-responses`, request projection also normalizes MCP argument schemas that use a non-object or root-level `oneOf`/`anyOf`/`allOf`/`enum`/`const`/`not`: Phoenix keeps the registered MCP schema unchanged, sends Codex an object-root compatibility view, and leaves the MCP server as the authoritative argument validator. Each retained image is preceded by stable text naming its complete attachment id and actual request dimensions. When accumulated base64 image payload exceeds the route's `maxRequestImageBytes`, each offloaded image (oldest first) is replaced by fixed text that tells the model to read the file again when a path is available or ask the user to attach it again. Offloaded normalized attachments are not read or transformed. Provider-native replay metadata is restored only when the adapter validates it for the historical content.
...
- **An unauthenticated route depends on its protocol** — naming no credential resolves the route as configured-but-keyless, but pi-ai's OpenAI-compatible implementation still requires an API key or an `Authorization` header. `chatgpt-web` supplies its non-secret loopback marker automatically; other keyless local servers need a placeholder credential referenced by `apiKeyEnv` or an `Authorization` entry in `headers`.
