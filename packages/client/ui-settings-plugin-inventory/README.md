...
The update action polls the repository-local `pluginInventory.updateState` Remote while Web is open. It consumes no sidebar space while PHOENIX is current. When a stable release is detected it appears immediately above Settings as a compact progress card with the stable channel, phase, target commit prefix, and an accessible progress value. It follows the updater's real lifecycle: source fetch, dependency preparation, build, smoke verification, ready, restart, activation, and rollback/error states. The card uses a clear state tint, progress track, target identifier, and stable-channel label; the compact sidebar rail renders the same state as an icon with a tooltip. The expanded sidebar shows localized text. The state is phase-based rather than a fabricated byte percentage.

Only `ready` becomes a restart action. The expanded card shows **Update ready** and **Restart now**; clicking it calls `pluginInventory.restartForUpdate`. The Host accepts that request only for the exact target already prepared by the detached stable updater. Once accepted, the UI moves to the restarting state, the Host exits, and the detached updater owns activation, rollback on failure, and automatic relaunch. Rejected or failed restart requests stay in the current process and refresh or show a generic update failure instead of closing PHOENIX. A temporary Host/RPC read failure is shown as **Checking for updates** and retried automatically; it is never mislabeled as a failed update. A durable updater error keeps the previous stable version visible and offers an immediate safe retry of the next state read.
