...
`ctx.layout` 还通过命名的 `subagent` 与 `cordis` 占用者协调共享的右侧可视工作区。第一个占用者会保存当前侧边栏和详情栏偏好、收起导航，并预留标准详情宽度；后续占用者共享同一租约，最后一个释放时精确恢复先前快照。租约活动期间，普通详情关闭请求不能收起该工作区，侧边栏不能展开覆盖它，并且即使没有普通详情 Session，AppFrame 也会保留右侧空间。详情栏拖动手柄在租约期间隐藏，使预留网格宽度与可视界面保持一致。
...
- **可视工作区在租约期间使用固定宽度**：子智能体与 Cordis 占用者共享约定详情宽度；未来的多窗格布局可以增加独立调整大小能力，而无需改变命名占用者的生命周期。
