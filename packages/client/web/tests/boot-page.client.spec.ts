...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    ...
    expect(el.textContent).toContain('PHOENIX HARDNESS')
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@phoenix-ai/dsh-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@phoenix-ai/dsh-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@phoenix-ai/dsh-client-ui-layout')
    expect(el.textContent).toContain('@phoenix-ai/dsh-client-ui-tool')
