# @phoenix-ai/dsh-client-ui-workspace
...
- **Session 删除是明确且物理的**：行菜单打开确认框并调用 \`session.delete\`；Host 拒绝活动会话，删除持久化 JSONL/SQLite 记录，清理 Workspace/归档记账并更新列表。删除不可撤销，因此没有取消归档入口；删除 Workspace 注册记录仍然保留其会话。
