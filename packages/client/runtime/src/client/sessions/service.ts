...
import { SESSION_SEARCH_RESULT_LIMIT } from '@phoenix-ai/dsh-host-apiproxy/api'
import type {
...
export class SessionRuntime implements ISessions {
  ...
  clear(): void {
    ...
    // destroys durable tab-local navigation intent. Transient list gaps must
    // never erase the selection while a page is still hydrating.
    this.selection.set({})
  ...
  private projectList(): void {
    ...
    // reconnects the tab-local selected session can be temporarily absent
    // from the list; preserve navigation intent until an explicit clear/delete
    // says otherwise.
