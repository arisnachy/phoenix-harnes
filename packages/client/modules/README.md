...
The host rejects a boot graph that omits either parser preload before emitting index HTML. This keeps a stale or incomplete build from reaching the browser as a page that can only fail later with a missing `client.js` preload.
...
- **No unload bookkeeping of its own** — style removal and fiber teardown ordering live with the HMR driver (`@phoenix-ai/dsh-client-hmr`); the loader only inventories owned style tag ids per record.
