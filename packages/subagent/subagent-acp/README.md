...
| `permission` | `reject` | Auto-answer permission requests by rejecting or choosing the first `allow_once` or `allow_always` option. `allow` requires `allowUnattendedPermissions: true`. |
| `allowUnattendedPermissions` | `false` | Explicit safety opt-in required before unattended `permission: allow` can activate. |
...
- **Permission prompts are auto-answered** (`permission: allow | reject`) — no human is surfaced a child's `session/request_permission`; unattended allow is blocked unless `allowUnattendedPermissions: true` is explicitly configured.
