...
Production resolves the `codex` bin declared by its pinned `@openai/codex@0.147.0` dependency and launches that JavaScript wrapper with the current Node executable. The wrapper selects the matching native platform payload; the provider neither inspects nor falls back to a host `codex` on `PATH`. Native Codex configuration and authentication remain authoritative through the parent cwd, `HOME`, and `CODEX_HOME`, while the Provider overrides only the selected thread approval/reviewer/sandbox fields. All other project, model, provider, MCP, hook, skill, and account settings remain native. The one-shot provider does not select a model, create `CODEX_HOME`, or log in. Credential-shaped ambient variables are removed by the subprocess seam before the explicit `env` overlay is applied.

When authorization and credential seams are present, the optional account bridge starts metadata-only app-server probes for account, quota, usage, and connector data. Those probes preserve the real `CODEX_HOME` but force the SQLite fallback to that same native home through both `CODEX_SQLITE_HOME` and a highest-priority CLI `sqlite_home` override. They deliberately ignore inherited or Profile-provided `PHOENIX_CODEX_SQLITE_HOME` and `CODEX_SQLITE_HOME` values for this metadata-only process. Older Phoenix runtimes used `CODEX_HOME/phoenix-runtime/sqlite/account`; on Windows that stale child environment can survive a Host handoff and restart the same cold 30-second backfill loop. One-shot delegated runs still keep their separate Phoenix SQLite state.
...
The standalone composition below shows the complete explicit capability. A Profile based on `@phoenix-ai/dsh-base` keeps its existing Job rows, adds the product provider and tool rows, and does not mount duplicate Job services.
...
- id: tool-jobs
  name: '@phoenix-ai/dsh-tool-jobs'

- id: tool-subagent-codex-safe
  name: '@phoenix-ai/dsh-tool-subagent'
  disabled: true
  config:
    provider: codex-safe
...
- **No human approval path** — known unattended approval requests are denied and unknown server requests fail closed; the three Profile modes never create a PHOENIX interaction channel or per-call allow policy.
