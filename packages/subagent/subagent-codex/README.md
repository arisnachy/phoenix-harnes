...
When authorization and credential seams are present, the optional account bridge starts metadata-only app-server probes for account, quota, usage, and connector data. Those probes preserve the real `CODEX_HOME` and reuse Codex's native SQLite selection: an explicit or inherited `CODEX_SQLITE_HOME` is passed through unchanged, otherwise Codex resolves its normal config/default location. Account probes deliberately do not use `PHOENIX_CODEX_SQLITE_HOME`; creating a fresh Phoenix-private account database would force Codex to backfill the user's complete rollout history before `account/rateLimits/read` is available. One-shot delegated runs keep their separate Phoenix SQLite state.
