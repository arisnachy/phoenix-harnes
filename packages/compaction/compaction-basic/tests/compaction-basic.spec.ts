...
import { selectCompactableRange } from '@phoenix-ai/dsh-compaction-basic/src/region.ts'
import type { SummarizationInput, SummaryResult } from '@phoenix-ai/dsh-compaction-basic/src/summarizer.ts'
import { CompactionId, toolPairingBalancedAfter, toolPairingBalancedBefore } from '@phoenix-ai/dsh-compaction'
import {
  resolveCompactSpec,
  resolveConfig,
  resolveTargetPolicy,
} from '@phoenix-ai/dsh-compaction-basic/src/config.ts'
import type { CompactionResult } from '@phoenix-ai/dsh-compaction'
import LlmRuntime, { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, createToolResultMessage, LlmAdapter , createMessage } from '@phoenix-ai/dsh-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmFailure,
...
import { agentEvents, type Agent, type RequestErrorAction } from '@phoenix-ai/dsh-agent'
...
describe('default one-shot summarizer', () => {
  ...
  it('uses configured model/default cap, forwards cancellation, and keeps only safe text', async () => {
    ...
    expect(instruction?.type === 'text' ? instruction.text : '').toContain('## Objective')
  })
  ...
  it('replays the conversation prefix and appends the instruction as the final message', async () => {
    ...
    expect(lastText).toContain('## Objective')
  })
