 ...
 * zero-build dev path, resolving `@phoenix-ai/dsh-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @phoenix-ai/dsh-loader-smoke
 */
...
// Windows cold boots the src/tsx product path more slowly than the lib path.
const DEFAULT_PROCESS_TIMEOUT_MS = process.platform === 'win32' ? 90_000 : 30_000

/** Vitest deadline that leaves room for the subprocess-owned process timeout. */
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated PHOENIX homes. */
