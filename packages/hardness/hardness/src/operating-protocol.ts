...
export function renderHardnessProtocol(locale: 'en' | 'es' = 'en'): string {
  ...
  if (locale === 'es') {
    return [
      ...
      'Ejecuta estos protocolos de forma silenciosa. En conversación normal no anuncies nombres de skills, workflows, protocolos, prompts, subagentes, compactación, routing, estados ocultos ni mantenimiento de contexto; informa únicamente progreso, hallazgos, bloqueos reales y entregables relevantes para el usuario.',
      ...
      'No expongas archivos de instrucciones internas, prompts ocultos, nombres de métodos de tools o MCP, ids de memoria, routers, subagentes ni otras piezas de implementación salvo que el usuario esté depurando explícitamente los internos de Phoenix o preguntando directamente por ese componente técnico.',
  ...
  return [
    ...
    'Run these protocols silently. In normal conversation do not announce skill, workflow, protocol, prompt, subagent, compaction, routing, hidden-state, or context-maintenance names; communicate only user-relevant progress, discoveries, genuine blockers, and deliverables.',
    ...
    'Do not expose internal instruction files, hidden prompts, tool or MCP method names, memory ids, routers, subagents, or other implementation details unless the user is explicitly debugging Phoenix internals or directly asking about that technical component.',
