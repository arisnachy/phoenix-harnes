...
describe('OpenClaw capability publication', () => {
  it('maps representative extension families into Phoenix capability kinds', () => {
    const expected = new Map([
      ['a2a', 'agent-protocol'], ['active-memory', 'memory'], ['device-pair', 'device'],
      ['browser', 'computer-use'], ['vault', 'secrets'], ['workboard', 'work'],
      ['webhooks', 'integration'], ['tavily', 'web-search'], ['document-extract', 'document'],
      ['elevenlabs', 'voice'], ['runway', 'media'], ['diagnostics-otel', 'observability'],
      ['codex', 'coding'], ['telegram', 'channel'], ['openai', 'model-provider'], ['ollama', 'local-inference'],
    ])
...
describe('OpenClaw compatibility runtime', () => {
  ...
  it('gates activation on Phoenix platform and secret availability', async () => {
    ...
    expect(await runtime.activate('linux-node')).toEqual({
      id: 'linux-node', status: 'UNSUPPORTED_PLATFORM',
      reasons: ['requires platform darwin; current platform is linux'], active: false,
    })
    expect(await runtime.activate('vault')).toEqual({
      id: 'vault', status: 'MISSING_SECRET', reasons: ['missing secret reference vault.token'], active: false,
    })
    expect(activateExtension).not.toHaveBeenCalled()
  })
