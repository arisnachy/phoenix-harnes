...
describe('PHOENIX capability operating protocol', () => {
  ...
  it('pins authoritative retrieval, current evidence, modality, automation, recovery, and low overhead', () => {
    ...
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('native computer tool')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('native image_generation')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('real raster asset')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('backend=codex')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('backend=free')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('never fake the image')
    expect(CAPABILITY_OPERATING_PROTOCOL).toContain('fresh screenshot as the source of truth')
