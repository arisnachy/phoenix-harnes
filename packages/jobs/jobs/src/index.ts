 ...
 * process-local registry lives in `@phoenix-ai/dsh-jobs-local`.
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@phoenix-ai/dsh-jobs is the abstract job registry seam; load an implementation such as @phoenix-ai/dsh-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
