...
npm scope: `@phoenix-ai/dsh-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).
...
| [`phoenix/`](phoenix/README.md) | PHOENIX downstream intelligence: capability ranking, adaptive routing, failover, efficiency, safety, and local evolution | Product — evolving API |
