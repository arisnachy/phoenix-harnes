...
Scope：DSH 使用 `@deepseek-ai/dsh-*`，PHOENIX 使用 `@arisnachy/phoenix-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名遵循各自的所有权 scope。**组 README 负责包／ctx 键映射。**
