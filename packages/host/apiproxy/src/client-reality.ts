...
export class ClientRealityService extends Service {
   ...
   * @param sessionId - Session that owns the ephemeral browser observation.
   * @param location - Browser-provided location and observation timestamp.
   */
  ...
  /**
   * Read the current non-expired position for one live or persisted session.
   * @param sessionId - Session whose latest browser observation should be read.
   * @returns A defensive copy of the current observation, or undefined when absent or expired.
   */
