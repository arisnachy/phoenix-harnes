...
import { isAppendSurfaceEvent, isJsonValue } from '@phoenix-ai/dsh-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@phoenix-ai/dsh-session'
import type { SessionPersistence } from '@phoenix-ai/dsh-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@phoenix-ai/dsh-session-query'
import { SubagentError } from '@phoenix-ai/dsh-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@phoenix-ai/dsh-subagent'
import { isUserInvocable } from '@phoenix-ai/dsh-skill'
import type { Workspace, WorkspaceRecord } from '@phoenix-ai/dsh-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@phoenix-ai/dsh-workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@phoenix-ai/dsh-agent-presets'
import type { PresetBearingSession } from '@phoenix-ai/dsh-agent-presets'
...
import type { SessionRawArtifact } from '@phoenix-ai/dsh-session-persistence'
import {
...
import type {} from '@phoenix-ai/dsh-session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@phoenix-ai/dsh-jobs'
...
import type {} from '@phoenix-ai/dsh-commands'
...
import type {} from '@phoenix-ai/dsh-cordis-host-runner/types'
import type {} from '@phoenix-ai/dsh-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@phoenix-ai/dsh-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@phoenix-ai/dsh-settings'
import { credentialRef, parseCredentialKey } from '@phoenix-ai/dsh-credentials'
import { AuthorizationDeclinedError, type AuthorizationPrompt } from '@phoenix-ai/dsh-authorization'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @phoenix-ai/dsh-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @phoenix-ai/dsh-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @phoenix-ai/dsh-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @phoenix-ai/dsh-credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @phoenix-ai/dsh-session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @phoenix-ai/dsh-skill', details: {} })
        }
