...
更新器状态位于 checkout 的 Git 目录中，不在 `$DSH_HOME`。Host 桥只接受文档规定的更新状态词汇，限制自由文本字段长度，只接受完整的 40 字符 commit id，并且绝不允许浏览器提供激活 target。重启请求会绑定到分离 stable 更新器已经写入的精确 `ready` target，随后 Host 只会在该请求持久写入后安排自身退出。Host 退出之后，激活、rollback 与重新启动都由更新器进程负责。
