...
Host diagnostics service for the current Cordis Loader tree and PHOENIX stable-update lifecycle. `PluginInventoryGateway` registers the `pluginInventory` Remote namespace. `pluginInventory/list` remains a read-only point-in-time projection of non-group Loader entries; `pluginInventory/updateState` exposes a sanitized snapshot of the repository-local updater state; `pluginInventory/refreshForUpdate` durably wakes the detached watcher for an immediate stable-channel check; `pluginInventory/restartForUpdate` accepts a restart only when that trusted state is exactly `ready` with a valid prepared target.

The ChatGPT Web lifecycle is also Host-owned. `pluginInventory/chatGptWebState`, `pluginInventory/enableChatGptWeb`, and `pluginInventory/disableChatGptWeb` expose a sanitized ON/OFF controller for the loopback browser bridge. The enabled preference is persisted separately from process ownership: only an explicit ON is restored after Host startup, health must reach a usable `/v1/models` response before Settings can expose the provider route, and browser credentials never cross this Remote boundary. On Windows, first-time ON discovers the current `codex-web-gpt-launcher` installation (plus legacy install paths), opens the installed Codex Web GPT launcher when Browser-only setup is still missing, and re-resolves setup/runtime state on later enable attempts without importing browser credentials into Phoenix.
...
Updater state lives under the checkout's Git directory, outside `$DSH_HOME`. The updater replaces this JSON document atomically, and the Host also retries a transient read race. The Host bridge recognizes only the documented updater status vocabulary, bounds free-text fields, accepts only full 40-character commit ids, and never lets the browser supply an activation target. A refresh request is a separate atomic marker consumed by the watcher, so the browser does not mistake rereading stale state for a new channel check. A restart request is bound to the exact `ready` target already written by the detached stable updater, then the Host schedules its own exit only after the request is durable. The updater process owns activation, rollback, and relaunch after the Host exits.
