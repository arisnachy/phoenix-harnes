 ...
 * @module @phoenix-ai/dsh-host-plugin-inventory/chatgpt-web
 */
...
export async function runChatGptWebBridge(args: readonly string[]): Promise<number> {
  ...
  try {
    const controller = new ChatGptWebBridge({ statePath: chatGptWebBridgeStatePath(), config: resolveChatGptWebConfig() })
