 ...
 *
 * @module @phoenix-ai/dsh-tool-fs-search/search-core
 */
...
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@phoenix-ai/dsh-subprocess'
 ...
 * (`@phoenix-ai/dsh-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@phoenix-ai/dsh-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@phoenix-ai/dsh-tool-fs-search/presentation} `grepSearchMeta`)
