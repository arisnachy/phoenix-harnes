...
function message(text: string, source: UserMessage['source']): UserMessage {
  return {
...
describe('cognitive snapshot feedback isolation', () => {
  it('keeps PHOENIX snapshots out of its own attention workspace', async () => {
    ...
    session.append('user/message', message(
      'Finish the OAuth mission and verify restart persistence.',
      { kind: 'user' },
    ), { surfaceOp: 'append' })
    ...
    session.append('user/message', message(snapshot, {
      kind: 'plugin',
