...
import * as SessionStatsPlugin from '@phoenix-ai/dsh-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@phoenix-ai/dsh-session-projection', SessionProjectionRegistry],
    ['@phoenix-ai/dsh-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@phoenix-ai/dsh-session-projection'",
      "- name: '@phoenix-ai/dsh-session-stats'",
    ])
