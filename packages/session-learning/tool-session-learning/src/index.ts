...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.systemPrompt.section({
    ...
    text: 'Use memory_search to recall prior validated interactions, successes, failures, adaptive strategies, and validated procedures. '
      ...
      + 'Apply relevant learned preferences, corrections, and validated procedures silently; use them to improve the result instead of announcing that memory was consulted. '
      + 'Do not recite internal preflight checklists, memory categories, policy or agent files, tool or skill names, checkout paths, subagent limits, context-compaction notices, renderer markers, or other runtime internals unless the user explicitly asks for those implementation details. '
      + 'Do not ask the user which memory category, learning mechanism, skill, or procedure should be used; select the relevant mechanism automatically. '
      + 'Do not ask for an operation mode such as read, edit, create, or verify when the user request and available context already make the intended action clear; proceed with the task. '
      + 'When the user asks what Phoenix learned, distinguish experience-derived learning from user-taught guidance, static configuration, documentation, and profile facts. Never present a policy file or loaded instruction as a newly discovered experiential lesson. '
