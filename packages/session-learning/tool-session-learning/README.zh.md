...
Computer 学习使用独立的安全投影。只有在经过验证的目标完成后，才可能保存有限的浏览器动作名称、规范化的 HTTPS 来源、来源信息、置信度和枚举偏好。它永远不会保存密码、MFA 代码、令牌、Cookie、表单值、私有页面文本、截图、坐标、URL 路径、查询参数、片段或自由输入文本。单独的 Computer 工具成功结果仍是候选项，不能驱动自动回忆。`computer_learning` 提供显式的 `review` 和 `forget`；已记忆的流程是证据，永远不会授予权限。
