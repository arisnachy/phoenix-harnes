...
可选的内置 skill（技能）提供方，向 `ctx.skills` 贡献保留兼容名称的 `dsh-badge`。该 skill 提供官方「powered by PHOENIX」Markdown 片段和随包分发的 PHOENIX PNG，供无法可靠导入远程图片的系统使用。
...
通过 `@phoenix-ai/dsh-tool-skill` 间接影响模型；该包会渲染目录条目和所选 skill 的正文。
