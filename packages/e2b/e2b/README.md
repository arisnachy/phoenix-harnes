...
```yaml
- id: e2b
  name: '@phoenix-ai/dsh-e2b'
  config:
    cwd: /home/user/workspace
    timeoutMs: 300000

- id: subprocess-e2b
  name: '@phoenix-ai/dsh-subprocess-e2b'

- id: fs-e2b
  name: '@phoenix-ai/dsh-fs-e2b'
```

`apiKey` is optional and otherwise reads `E2B_API_KEY`; the key configures the host SDK connection and is never installed in the sandbox. `cwd` defaults to `/home/user/workspace` and must be an absolute POSIX path. `timeoutMs` defaults to five minutes. `sandboxId` reconnects to an existing sandbox and renews its timeout. New sandboxes pause on timeout when `autoPause` is true; otherwise they are deleted.

## Lifecycle and ownership

Construction creates or reconnects to one sandbox. Before resolving `getSandbox()`, the service creates `cwd` and the private `cwd/.dsh-e2b` adapter-state directory, verifies that the reserved path is a real directory rather than a symlink or another file type, then sets it to mode `0700`. Each adapter-internal E2B command shell receives a fresh randomized root-level `HOME`, so the SDK's fixed login shell does not resolve profile files from the mutable user home before the control command.

Disposal first prevents new handle acquisition, then awaits setup and applies `retention`: `kill` deletes the sandbox (default), `pause` calls the SDK's `pause()` method, and `retain` leaves it untouched. A `SandboxNotFoundError` means the sandbox is already gone. Initial setup failure attempts deletion only for a newly created sandbox; a supplied sandbox is preserved. Provider plugins must load after this owner and dispose before it.
...
- **Retention depends on configuration** — default disposal deletes the sandbox; templates, volumes, and snapshots remain outside this POC.
