...
import { runLoaderSmoke } from '@phoenix-ai/dsh-loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
...
import { BashTerminalBackend } from '@phoenix-ai/dsh-terminal-bash'
  ...
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    try {
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> PHOENIX=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$DSH_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> PHOENIX=<> KEEP=<visible>')
