...
```yaml
- id: tool-agent-team
  name: '@phoenix-ai/dsh-experimental-tool-agent-team'
  config:
    freshProvider: spawn
    forkProvider: fork
    modelProfiles:
...
The generated [tool catalog](../../../docs/tool-catalog.md#phoenix-aidsh-experimental-tool-agent-team) owns exact schemas. The adapter supplies teammate creation; quiet and waking peer delivery; roster listing, waiting, and Lead-only interruption; and task create/list/get/compare-and-set update operations.
