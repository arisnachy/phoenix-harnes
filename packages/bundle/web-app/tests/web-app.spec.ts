...
import { createLaunchEnvironmentSnapshot, DSH_LAUNCH_ENVIRONMENT_KEY } from '@phoenix-ai/dsh-launch-environment'
import SystemPrompt from '@phoenix-ai/dsh-system-prompt'
import type { WebServer } from '@phoenix-ai/dsh-host-webserver'
...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('PHOENIX implementation checkout')
