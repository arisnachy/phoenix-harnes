...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/arisnachy/phoenix-harnes' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/arisnachy/phoenix-harnes/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
/**
 * The PHOENIX wordmark, inlined so its `currentColor` fills follow the active
...
/**
 * Navigation-bar title: the PHOENIX wordmark and the release-stage tag.
