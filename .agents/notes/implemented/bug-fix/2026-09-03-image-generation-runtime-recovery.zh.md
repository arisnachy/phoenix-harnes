...
PHOENIX 已暴露 `image_generation`，但多个彼此独立的检查仍可能阻止真实图像被尝试或接受。Codex 桥接层把当前 `doctor --json` 的表示形式当成权威依据，因此新的或尚未识别的诊断输出会在 `codex exec` 运行前被拒绝。HARDNESS 把所有已注册工具都投影为通用的 `tool` kind，因此像 `image_generation` 这样的语义需求无法选择 `tool:image_generation`。随后 acquisition 会把本地已经存在的工具误报为外部依赖。要求 `verified` 状态的需求无法执行刚准备好的 `testing` 能力来生成晋升所需的证据，而且生产 artifact runtime 也没有栅格图像 renderer。最后，受治理的恢复调用可能把视觉请求放在 `arguments.brief`/`arguments.objective` 中，而图像工具只接受 `prompt`，导致路由成功后马上出现第二次失败。
...
HARDNESS 将精确注册的工具 id `tool:<need.kind>` 视为该需求的语义 provider。精确名称工具路线以工具自己的 JSON schema 作为参数和输出的权威定义，而不会把自由文本任务描述误解释为 ATLAS 的 input/output 标签。Acquisition registry 可以把已经索引的精确名称 experimental/testing 工具准备为 `testing`，而不是错误地进入 `WAITING_EXTERNAL`；已经获得 `verified` 状态的工具绝不会被该 fallback 降级。

图像桥接层继续把 `prompt` 作为直接调用的规范字段，但也接受受治理恢复调用的别名，优先级为 `brief`，其次 `objective`。这样可兼容现有 `hardness_run` 的参数形状，而无需让通用 HARDNESS 内核理解图像专用的 prompt 语义。
...
**让通用 HARDNESS 重写图像参数。** 不采用，因为这会让 capability kernel 耦合到单一工具的 schema。图像 adapter 才是规范化 `prompt`/`brief`/`objective` 的正确边界。
