...
The base `dsh-system-prompt` assembly now also owns a fixed `harness:quality` section immediately after `harness:identity`. It asks the model to form a compact completion contract, reuse evidence already obtained, choose the cheapest reliable path, scale verification with complexity, uncertainty, consequence, and reversibility, avoid mandatory second-pass review for straightforward low-risk work, change strategy after failure, and validate artifacts on the surface where correctness is observable. It also forbids treating a successful tool return as sufficient proof of task completion. This remains prompt guidance rather than loop logic, so it applies across providers without adding an unconditional model call or changing the core agent loop.
...
The model prompt gains a bounded stable human-presence block, mission-continuity guidance, and a compact execution-quality contract, increasing prompt size slightly in exchange for consistent identity, interaction behavior, and completion discipline across providers without an unconditional review call. Automatic compaction becomes invisible in normal chat while remaining available to internal projections and explicit manual compaction. The three full shipped presets carry the same memory capability, so changing among them no longer drops recall context; the restricted `minimal` preset keeps its smaller contract. Memory, multimodal perception, learning, and automation remain capability-dependent, and recall uncertainty stays explicit rather than being converted into invented personal facts.
