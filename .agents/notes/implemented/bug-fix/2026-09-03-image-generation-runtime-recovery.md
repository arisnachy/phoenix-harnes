...
PHOENIX exposed `image_generation`, but several independent checks could prevent a real image from being attempted or accepted. The Codex bridge treated the current `doctor --json` representation as authoritative and rejected newer or unrecognized diagnostic output before `codex exec` ran. HARDNESS projected every registered tool under the generic `tool` kind, so a semantic need such as `image_generation` could not select `tool:image_generation`. Acquisition then reported an external dependency even though the tool already existed locally. A need requiring `verified` status could not execute a newly prepared `testing` capability to create the evidence required for promotion, and the production artifact runtime had no raster renderer. Finally, governed recovery calls could supply the visual request as `arguments.brief`/`arguments.objective`, while the image tool accepted only `prompt`, creating a second failure immediately after successful routing.
...
HARDNESS treats an exact registered tool id `tool:<need.kind>` as a semantic provider for that need. Exact-name tool routes use the tool's JSON schema as the argument/output authority rather than interpreting free-form mission descriptions as ATLAS input/output tags. The acquisition registry can prepare an already indexed exact-name experimental/testing tool as `testing` instead of declaring `WAITING_EXTERNAL`, while already verified tools are never downgraded by that fallback.

The image bridge keeps `prompt` as the canonical direct-call field but also accepts governed recovery aliases in priority order `brief`, then `objective`. This matches the existing `hardness_run` argument shape without teaching the generic HARDNESS kernel image-specific prompt semantics.
...
**Teach generic HARDNESS to rewrite image arguments.** Rejected because that would couple the capability kernel to one tool's schema. The image adapter is the correct normalization boundary for `prompt`/`brief`/`objective`.
