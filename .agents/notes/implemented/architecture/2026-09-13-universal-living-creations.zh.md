...
`@phoenix-ai/dsh-living` 是 Service Definition。`@phoenix-ai/dsh-living-local` 把 manifest 持久化到仅所有者可访问的版本化 JSON 目录中，并把运行提供者连接视为临时状态。因此重启后 Phoenix 会把已知创建物恢复为离线状态，而不是忘记它们。持久 manifest 变更通过同一条提交队列串行执行，并且只有在原子写入成功后才发布到内存，因此并发注册不会互相丢失，而失败的遗忘操作也不会把现有提供者断开。提供者连接必须通过真实方法证明实际达到的级别；无法满足 manifest 目标时会被拒绝。已有提供者连接时替换 manifest，新的契约必须仍与该提供者兼容；事件会依据最新已提交 manifest 校验，而不是依据最初挂载提供者时的旧契约。释放提供者只改变连接状态，不会删除持久身份。

`@phoenix-ai/dsh-tool-living` 是模型侧 Consumer。它的常驻 system-prompt 规则适用于 Phoenix 创建或实质修改的面向用户制品或可运行系统，不受领域或格式限制。工具负责注册与检查创建物、读取实时状态、执行已声明动作、验证目标级别与实际达到级别，并显式遗忘持久身份。只要创建物没有真正达到自己声明的目标，`living_verify_creation` 就会失败，从而避免模型仅因为文件或预览已经存在就把一个应当连接的系统宣布为完成。运行时丢失永远不代表删除；`living_forget_creation` 仅用于用户明确要求遗忘，或创建物已经永久删除且不再计划重连的情况。
...
本地提供者测试故意使用一种从未见过的创建类型，以证明系统不存在固定分类表；随后覆盖 manifest 持久化、由真实提供者推导集成级别、状态读取、动作、事件、释放后恢复离线、能力级别验证、并发持久变更、已挂载提供者条件下的 manifest 替换、依据最新 manifest 的事件校验，以及遗忘写入失败时的回滚语义。工具测试证明常驻规则是通用规则而不是类别枚举，证明在声明的实时目标真正连接之前完成验证会失败，并证明持久删除只能通过显式遗忘操作发生。共享 base bundle 会挂载本地提供者和模型 Consumer，因此 Phoenix 的正常 profile 都会继承这条规则。
