...
`@phoenix-ai/dsh-tool-pwsh` registers a Windows-only `computer` tool with the closed action vocabulary `screenshot`, `windows`, `focus`, `move`, `click`, `double_click`, `drag`, `type`, `key`, and `scroll`. `windows` enumerates visible top-level windows; `focus` resolves an explicit title, pid, or handle and verifies foreground identity. Input actions can carry the same selector as a guard, and state-changing actions capture a fresh post-action screenshot so the model verifies visible application state. Model-provided values travel only through environment variables into one fixed PowerShell/C# driver, while the driver itself is streamed over stdin to stay below the Windows command-line limit; neither path interpolates model text into executable source. The stdin payload includes the blank terminator required to execute its final compound statement, selects UTF-8 output, and validates every number against the signed 32-bit type used by the driver. The post-action delay removes its abort listener after normal settlement. Desktop authority derives from the existing sandbox policy instead of adding another durable permission knob: `read-only` is observe-only, while `workspace-write` and `danger-full-access` permit interaction. Interactive actions under `workspace-write` use the existing approval service; `danger-full-access` is the explicit no-prompt authority. Missing authority fails closed. Screenshots are stored through the optional attachment capability and deferred into model context as image content.
...
- Desktop input is impossible under observe-only authority, and the model receives only a closed validated action surface rather than arbitrary host commands. Window identity checks and fresh post-action screenshots reduce stale-coordinate and false-success failures.
