...
1. `@phoenix-ai/dsh-web` (`packages/web/web`) owns `ctx.web`, provider registration, provider selection, shared request/result vocabulary, and web-specific errors.
2. Provider packages implement concrete backends and register capabilities with `ctx.web`, for example `@phoenix-ai/dsh-web-search-exa`, `@phoenix-ai/dsh-web-search-perplexity`, `@phoenix-ai/dsh-web-search-deepseek`, and `@phoenix-ai/dsh-web-fetch-http`.
3. `@phoenix-ai/dsh-tool-web` (`packages/web/tool-web`) owns the model-facing `web_search` and `web_fetch` tool schemas, prompt sections, argument validation, result formatting, and tool-owned presentation over `ctx.web`.
...
```text
@phoenix-ai/dsh-tool-web  --depends on-->  @phoenix-ai/dsh-web  <--depends on--  @phoenix-ai/dsh-web-search-exa
        consumer                                 interface                       implementation
                                                                 <--depends on--  @phoenix-ai/dsh-web-search-perplexity
                                                                                  implementation
                                                                 <--depends on--  @phoenix-ai/dsh-web-search-deepseek
                                                                                  implementation
                                                                 <--depends on--  @phoenix-ai/dsh-web-fetch-http
                                                                                  implementation
```
...
```mermaid
flowchart LR
  exa["@phoenix-ai/dsh-web-search-exa"] -->|registerSearchProvider| web["@phoenix-ai/dsh-web / ctx.web"]
  perplexity["@phoenix-ai/dsh-web-search-perplexity"] -->|registerSearchProvider| web
  deepseek["@phoenix-ai/dsh-web-search-deepseek"] -->|registerSearchProvider| web
  fetchLocal["@phoenix-ai/dsh-web-fetch-http"] -->|registerFetchProvider| web
  toolWeb["@phoenix-ai/dsh-tool-web"] -->|search/fetch| web
  toolWeb -->|ctx.tools.register| webSearch["tool: web_search"]
  toolWeb -->|ctx.tools.register| webFetch["tool: web_fetch"]
```

`@phoenix-ai/dsh-web` depends only on Cordis and low-level harness support. It declares `ctx.web`, provider interfaces, request/result types, the provider availability contract, and error codes. It does not import tool, agent, session, LLM, or provider packages.
...
`@phoenix-ai/dsh-tool-web` depends on `@phoenix-ai/dsh-web`, `@phoenix-ai/dsh-tools`, `@phoenix-ai/dsh-system-prompt`, and Cordis. It never imports concrete provider packages.
...
```ts
import type { WebFetchRequest, WebFetchResult, WebSearchRequest, WebSearchResult } from '@phoenix-ai/dsh-web'
...
```yaml
- id: web
  name: '@phoenix-ai/dsh-web'
  config:
    searchProvider: exa
    fetchProvider: http

- id: web-search-exa
  name: '@phoenix-ai/dsh-web-search-exa'

- id: web-search-perplexity
  name: '@phoenix-ai/dsh-web-search-perplexity'

- id: web-search-deepseek
  name: '@phoenix-ai/dsh-web-search-deepseek'

- id: web-fetch-http
  name: '@phoenix-ai/dsh-web-fetch-http'
