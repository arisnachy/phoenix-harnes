...
面向模型的 scope 会增加一个很小且稳定的 prompt 区块，换来跨供应商更一致的能力使用。该变化应减少不必要的连接器发现、过时答案、猜测文件内容、重复工具调用，以及对视觉请求使用文字替代。实际执行仍取决于已挂载能力和现有审批、隐私、安全与验证边界；策略本身不会把不存在的 provider 变成真实能力。该变更还会防止模型仅因为 computer 或 image 工具不在显眼的顶层包名中，就忽略 Phoenix 已经存在的原生能力。
