...
The conversation composer owns an explicit browser-native voice-assistant mode. The user enables it through the microphone after the browser grants permission. Final recognition fragments become queued submissions, the recognizer restarts after browser segment termination and completed turns, and finalized assistant tails speak once through the local `speechSynthesis` adapter. Speech output removes code and link markup, selects the best matching installed natural or enhanced voice, and uses a moderate rate, pitch, and volume when the browser exposes those controls. A second microphone click cancels recognition and current speech. A durable activation timestamp and message key set prevent old transcript history and rerenders from being spoken again.

Human decisions use the same non-blocking interaction model. Every question receives a host-issued 60-second deadline, the browser shows the remaining seconds and automatic choice, and the host applies the deterministic recommendation even after reconnect or browser closure. Explicit option metadata or a recommendation suffix wins; a single-select confirmation without one uses a conservative label such as Cancel, No, Reject, Refuse, Stop, or Read-only, while multi-select chooses only explicitly recommended options.
...
The voice adapter tests recognition states, final-fragment aggregation, permission errors, activation fencing, speech deduplication, speech cleanup, voice selection, and speech lifecycle. InputBar tests verify automatic queue submission and that the assistant mode remains enabled after a final fragment. ChatView and conversation tests verify the finalized turn-tail integration. User-question, host, and UI tests verify the 60-second deadline, conservative automatic choice, reconnect replay, and countdown expiry. The focused regression suite passes with 48 tests, in addition to the existing 136-test voice-assistant coverage.
