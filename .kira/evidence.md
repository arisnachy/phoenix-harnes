...
- 2026-08-27 real approval integration: `createUserApprovalBroker` passed its focused test and typecheck; it calls `dsh-user-approval.request` with explicit agent, capability tool name, derived reason, and optional signal, mapping only `allowed-once` to a one-shot result. Adapter lint: 0 warnings/errors.
- 2026-08-27 governed evolution verification: `LabMode` and `SelfImprovementLedger` passed 3 focused tests; experiments cannot freeze without holdout validation, snapshots are immutable, and improvement records require rollback plus side-effects. Adapter typecheck and oxlint are clean.
- 2026-08-27 durable lab persistence: `JsonDocumentStore<T>` passed atomic JSON round-trip, while `LabMode.restore` and `SelfImprovementLedger.restore` passed reconstruction tests; atlas-json/adapters typechecks and oxlint are clean.
