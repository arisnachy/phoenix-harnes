...
const SESSION_PACKAGE = '@phoenix-ai/dsh-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@phoenix-ai/dsh-session/types'
 ...
 * top-level declaration (in `@phoenix-ai/dsh-session`) and any declaration
 * merge inside a `declare module '@phoenix-ai/dsh-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @phoenix-ai/dsh-session/known-event-types',
    ' */',
    '',
    '/**',
