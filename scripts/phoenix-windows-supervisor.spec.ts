...
describe('PHOENIX Windows updater supervisor resilience', () => {
  ...
  it('routes direct Windows web launches through the supervisor so restart survives the host exit', () => {
    ...
    expect(cliSource).toContain('PHOENIX_SUPERVISED_HOST_ARTIFACT: hostArtifact')
    expect(cliSource).toContain('PHOENIX_SUPERVISED_LAUNCH_CWD: process.cwd()')
    expect(cliSource).toContain('process.exit(result.status ?? 1)')
    expect(source).toContain("artifact === 'lib'")
    expect(source).toContain('[builtHostEntry]')
    expect(source).toContain("['--import', 'tsx/esm', sourceHostEntry]")
    expect(source).toContain('cwd: launchCwd')
  })
})
