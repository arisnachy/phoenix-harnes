...
if (!existsSync(manifestPath)) {
  ...
  for (const [key, value] of Object.entries(expected)) {
    if (manifest.policy?.[key] !== value) fail(`policy.${key} must remain ${value}`)
  }
...
const identityTouched = changed.some(path => identityFiles.has(path))
...
const riskSignals = [
  ...
  /\bRemove-Item\b[^\n]*-Recurse[^\n]*-Force/i,
  /\bsecrets?\b[^\n]*(?:log|print|console)/i,
].filter(pattern => pattern.test(added)).map(pattern => pattern.source)
if (riskSignals.length > 0 && !existsSync('docs/evolution/ROLLBACK.md')) {
  fail('high-risk candidate has no docs/evolution/ROLLBACK.md')
}

const sovereignPrefixes = [
  '.github/workflows/', '.github/CODEOWNERS', 'SECURITY.md',
  'packages/credentials/', 'packages/authorization/', 'packages/sandbox/',
]
