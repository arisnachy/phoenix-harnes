...
function main(): void {
  ...
  if (source === root && !values['verify-only']) {
    throw new Error('promote-client-artifacts: source and destination must differ')
  }
  ...
  if (!existsSync(sourceRecordPath)) {
    throw new Error('promote-client-artifacts: verified source build record disappeared')
  }
