...
describe('release families', () => {
  it('excludes private experimental packages from the dsh release', () => {
    ...
    expect(members.map(member => member.name)).not.toContain('@phoenix-ai/dsh-experimental-agent-team')
  })
  ...
  it('bumps private dsh packages without adding release tags', () => {
    ...
    const published = member('packages/core/published', '@phoenix-ai/dsh-published')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const dsh = releaseFamily('dsh')
    const members = [member('apps/cli', '@phoenix-ai/dsh'), { ...member('apps/web', '@phoenix-ai/dsh-web-frontend'), version: '0.0.2' }]
  ...
  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@phoenix-ai/dsh-consumer', { dependencies: { '@phoenix-ai/dsh-library': 'workspace:^' } }),
      member('packages/a/library', '@phoenix-ai/dsh-library'),
      member('packages/a/zebra', '@phoenix-ai/dsh-zebra'),
    ]

    expect(dsh.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@phoenix-ai/dsh-library',
      '@phoenix-ai/dsh-consumer',
      '@phoenix-ai/dsh-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@phoenix-ai/dsh-left', { dependencies: { '@phoenix-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@phoenix-ai/dsh-right', { dependencies: { '@phoenix-ai/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('publishes a peer before its consumer', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/consumer', '@phoenix-ai/dsh-consumer', { peerDependencies: { '@phoenix-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@phoenix-ai/dsh-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/left', '@phoenix-ai/dsh-left', { peerDependencies: { '@phoenix-ai/dsh-right': 'workspace:^' } }),
      member('packages/a/right', '@phoenix-ai/dsh-right', { peerDependencies: { '@phoenix-ai/dsh-left': 'workspace:^' } }),
    ]
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/base', '@phoenix-ai/dsh-base', { peerDependencies: { '@phoenix-ai/dsh-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@phoenix-ai/dsh-consumer', {
        dependencies: { '@phoenix-ai/dsh-base': 'workspace:^' },
        peerDependencies: { '@phoenix-ai/dsh-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@phoenix-ai/dsh-base', peer: '@phoenix-ai/dsh-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@phoenix-ai/dsh-alpha', { peerDependencies: { '@phoenix-ai/dsh-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@phoenix-ai/dsh-bravo', { peerDependencies: { '@phoenix-ai/dsh-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@phoenix-ai/dsh-charlie', { dependencies: { '@phoenix-ai/dsh-alpha': 'workspace:^' } }),
    ]
  ...
  it('ignores devDependencies when ordering', () => {
    const dsh = releaseFamily('dsh')
    const members = [
      member('packages/a/alpha', '@phoenix-ai/dsh-alpha', { devDependencies: { '@phoenix-ai/dsh-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@phoenix-ai/dsh-zebra'),
    ]
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('dsh').installedEntry).toEqual({ packageName: '@phoenix-ai/dsh', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@phoenix-ai/dsh-library', { files: ['lib/index.js'] }),
