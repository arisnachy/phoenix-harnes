...
import { applyChildComposition, childSessionMeta } from '@phoenix-ai/dsh-subagent'
import { CallId } from '@phoenix-ai/dsh-llm'
import type {} from '@phoenix-ai/dsh-compaction-basic'
...
import type {} from '@phoenix-ai/dsh-token-meter'
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@phoenix-ai/dsh-host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@phoenix-ai/dsh-client-ui-directory-picker-browse' },
    ] },
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    ...
    const handle = await ctx.agents.create({
      // Unique per run: the composition persists into the ambient PHOENIX home,
    ...
    try {
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by PHOENIX')
    } finally {
      await handle.dispose()
    }
  })
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    ...
    const packageName = (product: Product): string => (
      product === 'codex'
        ...
        : '@phoenix-ai/dsh-subagent-claude-code'
    )
