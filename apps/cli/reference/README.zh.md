...
组合包名称先从 dsh 安装目录解析，再从 profile 目录解析。因此，内置组合包（`@phoenix-ai/dsh-base`、`@phoenix-ai/dsh-web-app`、`@phoenix-ai/dsh-headless`）始终来自当前运行的 `dsh` 所属的安装；树外组合包则来自 profile 中由 pnpm 管理的 `node_modules`。patch 行中的裸插件 `name` 会从 profile 目录开始，按照 Node 的模块解析规则逐级向父目录查找，直至由 dsh 维护的安装后备目录 `$DSH_HOME/profiles/node_modules`。该目录为 dsh 安装中的应用和组合包所依赖的每个包各维护一个符号链接，并在每次启动时修复这些链接。
...
`dsh doctor` 在不加载用户 Profile 且不暴露凭据的情况下执行只读安装诊断。它检查 Node.js、Git、Python、仓库本地 updater 状态、Web 服务器响应以及 HTML client-module bootstrap。bootstrap 检查要求存在 `window.__ModuleLoader__` 和 Phoenix 命名空间的 `@phoenix-ai/dsh-client-modules/client.js`，并且不存在旧的 DeepSeek client-module 引用；因此过期或不完整构建的前端会以具体的检查名称报告为警告。
...
基础组合包挂载原生 DeepSeek 模型适配器、settings 与凭据提供方、由 OpenRouter 支持的稳定 `web_search`，以及已禁用的会话遥测。提供方凭据依次从继承环境、`$DSH_HOME/.credentials.yaml`、调用目录的 `.env` 和 `$DSH_HOME/.env` 解析；受管文档从不物化进 `process.env`，而两个 `.env` 文件都是普通启动环境层。搜索使用通过 **设置 → 模型** 保存的 `OPENROUTER_API_KEY`，并接受组合配置中的可选 OpenRouter 基址；只有 patch 层插入提供方并启用 `web_fetch` 后，该工具才可用。
...
通过 `dsh plugin --profile <name> add <package-or-git-spec>` 安装外部插件组合包。安装的包拥有其依赖，并贡献其声明的 `cordis.patch.yml` 层。CLI 还随附 `@phoenix-ai/dsh-mcp-client` 作为供 patch 层使用的依赖，但默认不启用 MCP 服务器，因为每条服务器命令都是 agent（智能体）沙箱之外的受信任可执行代码。
