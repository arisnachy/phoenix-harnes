...
Bundle names resolve from the dsh installation first, then from the profile directory. In-box bundles (`@phoenix-ai/dsh-base`, `@phoenix-ai/dsh-web-app`, `@phoenix-ai/dsh-headless`) therefore always come from the same installation as the running `dsh`; out-of-tree bundles come from the profile's pnpm-managed `node_modules`. A bare plugin `name` in any patch row resolves through the profile directory's Node parent-walk, which reaches the maintained installation fallback `$DSH_HOME/profiles/node_modules` (one symlink per package the installation's app and bundles depend on, healed on every launch).
...
```sh
dsh plugin --profile <name> add @phoenix-ai/dsh-subagent-codex
dsh plugin --profile <name> add @phoenix-ai/dsh-subagent-claude-code
dsh plugin --profile <name> add @phoenix-ai/dsh-subagent-codex @phoenix-ai/dsh-subagent-claude-code
dsh plugin --profile <name> remove @phoenix-ai/dsh-subagent-codex
dsh plugin --profile <name> remove @phoenix-ai/dsh-subagent-claude-code
```
...
`dsh doctor` performs read-only installation diagnostics without loading a user Profile or exposing credentials. It checks Node.js, Git, Python, the repository-local updater state, the Web server response, and the HTML client-module bootstrap. The bootstrap check requires `window.__ModuleLoader__`, the Phoenix-scoped `@phoenix-ai/dsh-client-modules/client.js`, and no legacy DeepSeek client-module reference, so a stale or partially rebuilt frontend is reported as a warning with a concrete check name.
...
The base bundle mounts the native DeepSeek model adapter, settings and credential providers, OpenRouter-backed stable `web_search`, and disabled session telemetry. Provider credentials resolve from the inherited environment, `$DSH_HOME/.credentials.yaml`, the invoking directory's `.env`, then `$DSH_HOME/.env`; the managed document is never materialized into `process.env`, while both `.env` files are ordinary launch environment layers. Search uses `OPENROUTER_API_KEY`, saved through **Settings → Models**, and accepts an optional OpenRouter base URL in the composition config; `web_fetch` is disabled unless a patch layer inserts a provider and enables it.
...
Install external plugin bundles through `dsh plugin --profile <name> add <package-or-git-spec>`. The installed package owns its dependencies and contributes its declared `cordis.patch.yml` layer. The CLI also ships `@phoenix-ai/dsh-mcp-client` as a dependency for patch layers, but no MCP server is enabled by default because each server command is trusted executable code outside the agent sandbox.
