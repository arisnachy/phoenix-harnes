...
## Do not import `@phoenix-ai/dsh-client-*` here
...
`@phoenix-ai/dsh-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@phoenix-ai/dsh-client-web` and the boot-manifest
type from `@phoenix-ai/dsh-client-modules/client`: booting the real shell is what
...
`@phoenix-ai/dsh-client-runtime/client` because `client/runtime` is reachable
