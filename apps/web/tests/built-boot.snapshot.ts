...
it('boots the built plugin graph and renders a fixture session end to end', async () => {
  ...
  expect(document.querySelector('[role="img"][aria-label="PHOENIX"]')?.textContent).toBe('PHOENIX')
  expect(document.querySelector('img[src="/phoenix-emblem.png"]')).not.toBeNull()
  expect(screen.queryByText('PHOENIX Local Build')).toBeNull()
  ...
  for (const plugin of ['@phoenix-ai/dsh-client-ui-layout', '@phoenix-ai/dsh-client-ui-sidebar', '@phoenix-ai/dsh-client-ui-conversation', '@phoenix-ai/dsh-client-ui-tool']) {
