...
import { bootInjections, orderByModuleGraph } from '@phoenix-ai/dsh-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@phoenix-ai/dsh-client-modules/client'
import { AppWebEntry } from '@phoenix-ai/dsh-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@phoenix-ai/dsh-app-boot')).href) as unknown as BootComposition
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  for (const id of ['@phoenix-ai/dsh-client-modules', '@phoenix-ai/dsh-client-runtime']) {
