...
internal static class DesktopRuntimeLaunchContract
{
        ...
        bool showDeveloperConsole = false)
    {
        ...
        // Consumer desktop startup prioritizes an already-built Host and defers expensive
        // recovery validation until it is actually needed. Developer console keeps the full
        // source/preflight path for diagnostics.
...
internal static class DesktopStartupProgress
{
    internal static string? FromRuntimeLine(string? line)
    {
        ...
        if (value.Contains("http://127.0.0.1:3080", StringComparison.OrdinalIgnoreCase)
            || value.Contains("127.0.0.1:3080", StringComparison.OrdinalIgnoreCase))
